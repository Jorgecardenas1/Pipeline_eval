...
def testwithLabels(netG,device):
    ...
    array_labels, noise,data_raw = prepare_data_arbitrary(data,sustratoHeight, device,name)

    testTensor = noise.type(torch.float).to(device)

    if parser.GAN_version:
        label_conditions = torch.stack(array_labels).type(torch.float).to(device) #Discrminator Conditioning spectra
        noise = noise.type(torch.float).to(device) #Generator input espectro+ruido
        label_conditions = torch.nn.functional.normalize(label_conditions, p=2.0, dim=1, eps=1e-5, out=None)
    ...
    with open(parser.output_path+"/usedmodel.txt", "a") as f:
        f.write("original batch:Arbitrary")
        f.write("\n")
        ...
        f.write("cell size:"+str(cellsize)+" "+"ValidationWidth:N.A.")
        f.write("\n")

    df_ = pd.DataFrame(t_np) #convert to a dataframe
    df_real = pd.DataFrame(data_raw)
    #save read data
    df_.to_csv(parser.output_path+"/eval_data",index=True) #save to file
    df_real.to_csv(parser.output_path+"/spectra"+str(name),index=True) #save to file

    plt.plot(data_raw[0], data_raw[1])
        # Saving figure by changing parameter values
    plt.savefig(parser.output_path+"/spectra"+str(name)+".png")
...
def prepare_data_arbitrary(CSV_labels,substrateH, device,target):
    ...
    values_array=torch.Tensor(TargetGeometries[target])
    values_array=torch.cat((values_array,torch.Tensor([substrateH ])),0)

    """ conditions no top frequencies"""
    conditional_data = torch.Tensor(values_array)
    ...
    return array_labels, noise,data_raw
