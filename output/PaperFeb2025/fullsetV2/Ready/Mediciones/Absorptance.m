...
files = struct(...
    'angle0', struct('absorber', 'abs6_band.s2p', 'reflect', 'Reflection0Deg.s2p'), ...
    'angle10', struct('absorber', 'abs10degband.s2p', 'reflect', 'Reflectioncontrol10deg.s2p'), ...
    'angle20', struct('absorber', 'abs20degband.s2p', 'reflect', 'Reflectioncontrol20deg.s2p'), ...
    'angle30', struct('absorber', 'abs30degband.s2p', 'reflect', 'Reflectioncontrol20deg.s2p') ...
        );
...
abs30         = processAngle(files.angle30.absorber, files.angle30.reflect, 20);

% ---------- Export to CSV ----------
T = table(freq, abs0, abs10, abs20, abs30, ...
    'VariableNames', {'Frequency_GHz', 'Abs0deg', 'Abs10deg', 'Abs20deg','Abs30deg'});
...
function [P_absorbed_normalized, freq_GHz] = processAngle(absorber_file, reflect_file, angle_deg)
    ...
    ref = 2e3;
    P_in = 1;  % normalized
    P_reflected = (abs(s11_absorber) + abs(s11_reflect)).^2 ./ abs(s11_absorber).^2;
    P_absorbed_normalized =   (P_reflected / ref);
